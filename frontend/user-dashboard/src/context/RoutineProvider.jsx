...
function routineReducer(state, action) {
  switch (action.type) {
    ...
    case 'UPDATETASKISCOMPLETE':
      ...
      if (up_task.isCompleted === true) return { ...state, tasks: up_tasks, score: up_score }
      else return { ...state, tasks: up_tasks }

    case 'ADDSUBTASK':
      const new_weightage = action.payload.subTask.weightage
      const updatedTasks = state.tasks.map((task, index) => {
        if (index == action.payload.index) {
          const oldSubTasks = task.subTasks
          //?updating the new weightage according to the the number of subTasks
          let newSubTasks = oldSubTasks.map((subTask) => {
            return { ...subTask, weightage: new_weightage }
          })
    ...
    case 'UPDATESUBTASK':
      ...
      const updated_sub_task = { ...selected_sub_task, description:action.payload.subTask.description }
    ...
    case 'DELETESUBTASK':
      ...
      let newSubTasks = task.filter((subTask, index) => index !== action.payload.subtaskIndex)
      const lengthOfSubTasks = newSubTasks.length
      const newWeightage=100/lengthOfSubTasks
      //?updating the new weightage according to the the number of subTasks
      newSubTasks=newSubTasks.map((subTask)=>{return {...subTask, weightage:newWeightage}})
    ...
    case 'UPDATESUBTASKISCOMPLETE':
      ...
      const up_subTasks = sele_task.subTasks.map((subTask, index) => {
        if (subTask._id === action.payload.subTaskId) return up_subTask
        return subTask
      })

      const new_updated_task = { ...sele_task, subTasks: up_subTasks }
      const new_updated_tasks = state.tasks.map((task) => {
        if (task._id === action.payload.taskId) return new_updated_task
        return task
      })
...
function RoutineContextProvider({ children }) {
  ...
  function updateSubTask(taskIndex, subtaskIndex, subTask) {
    dispatch({
      type: 'UPDATESUBTASK',
      payload: { taskIndex: taskIndex, subtaskIndex: subtaskIndex, subTask: subTask }
    })
  }
  ...
  return <RoutineContext.Provider value={values}>{children}</RoutineContext.Provider>
}
