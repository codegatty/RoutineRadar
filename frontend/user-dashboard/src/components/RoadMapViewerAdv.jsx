...
export const RoadMapViewerAdv = () => {
  ...
  function isCompleteHandler(updatedData){
    ...
    const updatedArrayOfCompletion=arrayOfComplete.map((ele,idx)=>{
      if(idx==updatedData.index)
        return updatedData.isEnabled 
      return ele
    })
  ...
  return (
     ...
     >
      <option value={null}>Select a roadMap</option>
      {
        roadMapCtx.roadMaps.map((roadMap,idx)=>{
          return (
            <option value={roadMap._id} key={idx}>{roadMap.title}</option>
          )
        })
      }
    </Select>
    ...
    <div className="flex flex-col  h-full w-full my-4  overflow-scroll scrollbar-thin scrollbar-thumb-secondary scrollbar-track-primary">
...
const Circle = ({arrayOfComplete,index,onUpdateIsComplete}) => {
  ...
  return <div className={classNames("rounded-full w-6 h-6 p-1   hover:bg-secondary mx-auto mb-3 flex justify-center content-center",isEnabled?"bg-secondary":"bg-primary")}>
...
const Pillar = ({isEnabled}) => {
  return <div className={classNames("rounded-t-full rounded-b-full w-1 h-20 border border-5 border-secondary  mx-auto",isEnabled?"bg-app-blue":"bg-primary")}></div>
}
