...
function ProgressViewer({ className, userData }) {
  ...
  return (
    <div className={classNames(className, 'flex items-center justify-center flex-col py-10  h-full')}>
      <div className="flex flex-col w-96  rounded-md shadow-sm shadow-secondary ">
        ...
        <div className="flex flex-row my-3 mx-1">
          <label className="flex-1 text-center font-bold text-primary"> Completed challenges </label>
          <span className="flex-1 bg-app-blue text-center rounded-lg font-bold text-secondary">
            {userData?.participatedChallengeIds.length}
          </span>
        </div>
      </div>

      <div className="flex flex-col gap-2 my-3 mx-1  bg-secondary   p-1   rounded-md shadow-sm shadow-secondary fill-emerald-50 ">
